...
export default function PhotosUploader({ addedPhotos, onChange, id }) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      fetch("https://api.cloudinary.com/v1_1/dgau25rss/image/upload", {
        ...
        .then((data) => {
          console.log(data);
          onChange((prev) => {
            return [...prev, { url: data.url, public_id: data.public_id }];
          });
        });
    }
  }
  function removePhoto(ev, photo) {
    ev.preventDefault();
    onChange([...addedPhotos.filter((p) => p.public_id !== photo.public_id)]);
    axios.put("/delete-photo", { photo, id });
  }
